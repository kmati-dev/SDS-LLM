...
Analyses:
  ...
  4. Speedup vs lexical overlap scatter       — full dataset
...
def compute_all_data(tokenizer, ds_raw):
    ...
    Returns a dict with:
      speedups, zero_drafts, overlaps        — per-sample lists
    ...
    for idx in tqdm(range(start_idx, n_total), desc="Computing", unit="sample",
        ...
        if (idx + 1) % CKPT_INTERVAL == 0:
            _save_checkpoint(CHART_CHECKPOINT, {
                "speedups": speedups, "zero_drafts": zero_drafts, "overlaps": overlaps,
...
def print_single_sample_analysis(tokenizer, corpus_text, target_text, question):
    corpus_tokens = tokenizer.encode(corpus_text)
    target_tokens = tokenizer.encode(target_text)
    overlap       = compute_overlap(corpus_tokens, target_tokens)

    print("\n" + "=" * 65)
    print("SQuAD ROOT CAUSE ANALYSIS  (deep dive — sample #0)")
    print("=" * 65)
...
def print_multi_sample_stats(data):
    ...
    print(f"  Overlap   : mean={np.mean(overlaps):.1%}  std={np.std(overlaps):.1%}")
...
def plot_charts(data):
    ...
    plt.suptitle("SQuAD Analysis — Part 1 (full dataset)", fontsize=14, fontweight="bold")
    plt.tight_layout()
    ...
    # Panel 2 — Speedup vs overlap scatter (subsampled for visibility)
    ax = axes[1]
    rng = np.random.default_rng(42)
    n_scatter   = min(SCATTER_MAX, len(overlaps))
    scatter_idx = rng.choice(len(overlaps), size=n_scatter, replace=False)
    sc_ov = [overlaps[i]  for i in scatter_idx]
    sc_sp = [speedups[i]  for i in scatter_idx]
    ax.scatter(sc_ov, sc_sp, color=COLOR, s=30, alpha=0.5, edgecolors="none")
    ax.set_title(f"Speedup vs Lexical Overlap\n({n_scatter} sampled from {n_samples:,})",
                 fontsize=11, fontweight="bold")
    ...
    plt.suptitle("SQuAD Analysis — Part 2 (full dataset)", fontsize=14, fontweight="bold")
    plt.tight_layout()
...
def main():
    ...
    # Text output
    print_single_sample_analysis(tokenizer, corpus_text, target_text, question)
    print_multi_sample_stats(data)
